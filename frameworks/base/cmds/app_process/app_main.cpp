...
int main(int argc, char* const argv[])
{
    ...
    // 创建一个Runtime 在Zygote进程里面有个Runtime对象，所有的Java进程内部都有一个Java虚拟机
    ...
    if (!className.isEmpty()) {
        ...
        // 在Zygote模式下，将所有剩余参数传递给合子main（）方法。
        for (; i < argc; ++i) {
    ...
    if (zygote) { // Zygote模式，启动Zygote Init进程，带参数，加Zygote标记
        runtime.start("com.android.internal.os.ZygoteInit", args, zygote);
    } else if (!className.isEmpty()) { // 启动其他的Java 非Framework进程
